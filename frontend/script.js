...
async function loadExperiences() {
    try {
        // ONLY try real Dojo API - NO FALLBACKS
        const response = await fetch(`${API_BASE_URL}/dojo/experiences`);
        const data = await response.json();
        
        if (!response.ok || !data.success || !data.data) {
            throw new Error(data.error || data.message || 'Failed to load experiences from Dojo API');
        }

        availableExperiences = data.data;
        console.log('Experiences loaded from Dojo API:', availableExperiences);
        populateExperienceSelect();
    } catch (error) {
        console.error('Error loading experiences:', error);
        showError('Failed to load dining experiences from Dojo. Please check your API connection.');
...
async function loadMenuItems() {
    ...
    try {
        showLoading(true);
        const response = await fetch(`${API_BASE_URL}/dojo/experiences/${bookingData.experience_id}/menu`);
        const data = await response.json();
        ...
        <div class="category">
            ...
            <div class="menu-items">
                ${category.items.map(item => `
                    <div class="menu-item">
                        <input type="checkbox" 
                               id="person-${personNumber}-item-${item.id}" 
                               name="person-${personNumber}-items" 
                               value="${item.id}">
                        <label for="person-${personNumber}-item-${item.id}">
                            ${item.name}
...
async function submitBooking() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/dojo/bookings`, {
            method: 'POST',
            headers: {
...
async function submitEventPreorder() {
    ...
    try {
        ...
        if (response.ok) {
            showEventConfirmation(result.booking);
        } else {
